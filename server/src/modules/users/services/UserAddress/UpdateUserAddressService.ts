...
class UpdateUserAddressService {
  public async execute({
    ...
    if (!user.shippingAddresses)
      throw new AppError(
        "User doesn't have registrered any address.",
        statusCodes.badRequest,
      );

    if (main) {
      const oldMainShippingAddress = user.shippingAddresses.find(
        ({ main }) => main === true,
      );

      if (oldMainShippingAddress) oldMainShippingAddress.main = false;
    }

    const shippingAddressToUpdate = user.shippingAddresses.find(
      shippingAddress => shippingAddress.postalCode === oldPostalCode,
    );
    ...
    if (postalCode) {
      const samePostalCodeAddress = user.shippingAddresses.find(
        shippingAddress => shippingAddress.postalCode === postalCode,
      );

      if (samePostalCodeAddress && oldPostalCode !== postalCode)
        throw new AppError(
          'Postal code already registered.',
          statusCodes.badRequest,
        );
    }
    ...
    shippingAddressToUpdate.address =
      address ?? shippingAddressToUpdate.address;
    shippingAddressToUpdate.country =
    ...
    shippingAddressToUpdate.postalCode =
      postalCode ?? shippingAddressToUpdate.postalCode;
